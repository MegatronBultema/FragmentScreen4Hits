...
def uploaded_file(filename):
    if request.method == 'POST':
        ...
        rffit, y_predict = rf.randomforest(X_train.values, X_test, y_train.values, y_test)
...
#f_descrip = feature_description.to_html(classes = "table table-condensed")
@app.route('/hit')
def uploaded_HTS(filename = 'MTH1_HTS_5500.csv'):
    ...
    scored_hts = hts_a.score_HTS(rffit, features, ids)
    ...
    roc.plotHTS_roc(features, y, y_proba, 'Test_HTS')
    precision, recall, fpr, fpr_test, tpr_test, cm = hts_a.print_threshold(rffit, features, y, mrecall_threshold)
    cm = pd.DataFrame(cm)
