...
def tune_prob_cutoff(model, X_train, X_test, y_train, y_test , return_vals = True):
	...
	if return_vals == True:
		l_ind = [i for i,m in enumerate(my_pred) if m==1]
		X_testp = X_test.iloc[l_ind,:]
		y_testp = [list(y_test)[i] for i in l_ind]
	return X_testp.values, y_testp
...
if __name__ == '__main__':
	...
	X_trainp_over, y_trainp_over = tune_prob_cutoff(estimator, X_train_over,X_test, y_train_over, y_test , return_vals = True)
	'''
