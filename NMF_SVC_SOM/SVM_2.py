...
def grid_scv(X_train, X_test, y_train, y_test):
    ...
    param_grid = {'C': [1, 1e3, 1e4, 1e5],
              'gamma': [0.0001, 0.01, 0.1, 1, 1e1], 'degree':[3,4,5] }
    clf = GridSearchCV(SVC(class_weight='balanced', kernel = 'rbf'), param_grid, scoring = 'roc_auc', verbose = 1)
...
if __name__ == '__main__':
    ...
    #scaler, features_scaled = scale(features)
    f1X_train, f1X_test, f1y_train, f1y_test = train_test_split(f1_scaled, yfill, test_size=0.20, random_state=1, stratify =yfill)
    print('f1 grid search:')
    grid_scv(f1X_train, f1X_test, f1y_train, f1y_test)
