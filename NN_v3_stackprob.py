...
def tune_prob_cutoff(model, X_train, X_test, y_train, y_test , stack_prob = True):
	# should add in at parameter of acceptable recal vs precission
	# then can iterate through thresholds and choose the one that allows for the desired ration of TP too FP. Right now 0.05 gives a recall of 0.78 and precision of 0.23
	'''
	[['TN' 'FP']
	 ['FN' 'TP']]
	[[59 23]
	...
	pprob = model.predict_proba(X_test.values)
	pdf = pd.DataFrame(pprob)
	print(pdf)
	pdf['myH'] = pdf[1].map(lambda x: 1 if x>.01 else 0)
	...
	if stack_prob == True:
		pprob_Xtrain = pd.DataFrame(model.predict_proba(X_train))
		#print(pprob_Xtrain)
		pprob_Xtest = pd.DataFrame(model.predict_proba(X_test.values))
		#print(pprob_Xtest)
		ppX_train = pd.DataFrame(X_train)
		ppX_test = pd.DataFrame(X_test.values)
		ppX_train['prob_NN'] = pprob_Xtrain[1].map(lambda x: 1 if x>.01 else 0)
		#print(ppX_train)
		ppX_test['prob_NN'] = pprob_Xtest[1].map(lambda x: 1 if x>.01 else 0)
		#print(ppX_test)
		#below is used to add predicted probability to data
		#ppX_train['prob_NN'] = pprob_Xtrain[:,1]
		#ppX_test['prob_NN'] = pprob_Xtest[:,1]
		print('Probability of hit from NN added to X_train and X_test data in output')
	return ppX_train.values, ppX_test.values
...
if __name__ == '__main__':
	...
	pX_train, pX_test = tune_prob_cutoff(estimator, X_train_over,X_test, y_train_over, y_test , stack_prob = True)



	'''
	estimator2 = KerasClassifier(build_fn=define_model2, nb_epoch=300, verbose=1, batch_size =30)
	estimator2.fit(X_trainp_over, y_trainp_over)
	pprob = estimator2.predict_proba(X_test.values)
	pdf = pd.DataFrame(pprob)
	print(pdf)
	pdf['myH'] = pdf[1].map(lambda x: 1 if x>.35 else 0)
	my_pred = pdf['myH'].values
	print(my_pred)
	print("precision:", precision_score(y_test, my_pred))
	print("recall:", recall_score(y_test, my_pred))
	...
	NOTE: training on 2 NN returned a confusion matrix that was not much better
	'''
	...
	#X_testp = tune_prob_cutoff(estimator, X_train, X_test, y_train, y_test , return_vals = True)
